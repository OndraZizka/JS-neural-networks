...
cNeuralNetTeacher.prototype = {
	...
	Turn: function(){
		...
		_dw.Write("Vstupní hodnoty: [" + this.oCurTrainSetItem.adI + "]"); ///
		_dw.Write("Chtìné  hodnoty: [" + this.oCurTrainSetItem.adO + "]"); ///
	...
	PerformCorrection: function(){
		...
		while(aoCurNeurons.length > 0) {
			...
			for( var i in aoCurNeurons ){
				...
				setDoneNeurons.put(oCurNeuron);     // Pøidáme zpracovávaný neuron mezi hotové (má spoètený error). 
